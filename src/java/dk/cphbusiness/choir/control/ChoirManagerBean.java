...
public class ChoirManagerBean implements ChoirManager {
    @PersistenceContext(unitName = "ChoirBackendPU")
...
//        em.getTransaction().begin();
        ...
        if (material.getVoices() != null) {
            for (VoiceSummary voiceCode : material.getVoices()) {
                voices.add(em.find(Voice.class, voiceCode.getCode()));
            }
        }
        if (material.getType() != null) {
            if (material.getType().equals("sheet")) {
                sheet = new Sheet();
                sheet.setId((int) material.getId());
                sheet.setPageCount(material.getPageCount());
                sheet.setFile(material.getFileName());
                sheet.setFileSize(material.getFileSize());
                sheet.setVoices(voices);
                sheet.setMusic((em.find(Music.class, (int) material.getMusicId())));
                sheet.setTitle(material.getTitle());
                if (sheet.getId() == 0) {
                    em.persist(sheet);            //Creates new material if it doesn't already exist in DB
                } else {
                    em.merge(sheet);            //Updates if material already exists in DB
                }
            } else {
                audio = new Audio();
                audio.setId((int) material.getId());
                audio.setPlayingTime(material.getPlayingTime());
                audio.setFile(material.getFileName());
                audio.setMusic(em.find(Music.class, (int) material.getMusicId()));
                audio.setFileSize(material.getFileSize());
                audio.setTitle(material.getTitle());
                audio.setVoices(voices);
                if (audio.getId() == 0) {
                    em.persist(audio);            //Creates new material if it doesn't already exist in DB
                } else {
                    em.merge(audio);            //Updates if material already exists in DB
                }
            }
        }
...
//            saveArtist.setDateOfDeath(artist.getDateOfDeath());
            ...
            if (artist.getId() == 0) {
                ...
                em.merge(saveArtist);            //Updates if member already exists in DB
            }
